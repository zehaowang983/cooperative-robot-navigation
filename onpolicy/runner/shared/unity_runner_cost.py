...
import numpy as np
import torch
from onpolicy.runner.shared.base_runner_cost import CRunner
from onpolicy.utils.mpe_runner_util import *

def _t2n(x):
    return x.detach().cpu().numpy()

class MPECRunner(CRunner):
    """Runner class to perform training, evaluation. and data collection for the MPEs. See parent class for details."""
    def __init__(self, config):
        super(MPECRunner, self).__init__(config)
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            for step in range(self.episode_length):
                ...
                # Sample actions
                values, cost_values, actions, action_log_probs, rnn_states, rnn_states_critic, actions_env, rnn_states_cost_critic = self.collect(step)
                ...
                if self.use_gt_ogm:
                    ...
                    ogm_pred, costs = self.cooperative_prediction(lidar, vec_obs)
                ...
                data = obs, rewards, costs, dones, infos, values, cost_values, actions, action_log_probs, rnn_states, rnn_states_critic, rnn_states_cost_critic
            ...
            if episode % self.log_interval == 0:
                ...
                print("average episode costs is {}".format(train_infos["average_episode_costs"]))
                avg_reward_message = "average episode rewards is {}\n".format(
                np.mean(self.buffer.rewards) * self.episode_length)
                avg_reward_message += "average episode costs is {}\n".format(
        ...
        if self.collect_data:
            for agent_id in range(self.num_agents):
                np.save(os.path.join(self.save_data_dir, f"all_lidar_data_{agent_id}_val.npy"), all_lidar_data[agent_id])
                np.save(os.path.join(self.save_data_dir, f"all_pos_data_{agent_id}_val.npy"), all_pos_data[agent_id])
                np.save(os.path.join(self.save_data_dir, f"all_traj_length_{agent_id}_val.npy"), all_traj_length[agent_id])
    ...
    def prepare_obs(self, obs):
        ...
        # vec_obs = obs[:,:,:,c_ogm*h_ogm*w_ogm+n_points:c_ogm*h_ogm*w_ogm+27+n_points].reshape(n_rollout_threads, num_agents, vis_seq, -1) # n_rollout_threads, num_agents, vis_seq, 27 
        # human_obs = obs[:,:,:,c_ogm*h_ogm*w_ogm+27+n_points:].reshape(n_rollout_threads, num_agents, vis_seq, -1) # n_rollout_threads, num_agents, vis_seq, 9*npc

        lidar = obs[:,:,:,:n_points].reshape(n_rollout_threads, num_agents, vis_seq, -1) # n_rollout_threads, num_agents, vis_seq, 1, 360
        # vec_obs = obs[:,:,:,n_points:n_points+27].reshape(n_rollout_threads, num_agents, vis_seq, -1) # n_rollout_threads, num_agents, vis_seq, 27 
        # human_obs = obs[:,:,:,n_points+27:].reshape(n_rollout_threads, num_agents, vis_seq, -1) # n_rollout_threads, num_agents, vis_seq, 9*npc
        ...
        vec_obs = obs[:,:,:,n_points:n_points+self.num_agents*9].reshape(n_rollout_threads, num_agents, vis_seq, -1) # n_rollout_threads, num_agents, vis_seq, 27 
        human_obs = obs[:,:,:,n_points+self.num_agents*9:].reshape(n_rollout_threads, num_agents, vis_seq, -1) # n_rollout_threads, num_agents, vis_seq, 9*npc
        ...
        return None, None, None, lidar, None, None, vec_obs, human_obs, None
    
    def cooperative_prediction(self, lidar, vec_obs, is_eval=False, step=0):
        ogm_obs = []
        ...
        for agent_id in range(self.num_agents):
            ...
            # fig, axes = plt.subplots(2, self.vis_seq, figsize=(15, 10))
            # for col in range(self.vis_seq):  # Loop through columns
            #     axes[0, col].imshow(ego_ogm[0, col], origin='lower')  # Replace with actual ego_ogm[0, col] if needed
            #     axes[0, col].set_title(f"ego_ogm[0, {col}]")
            #     axes[0, col].axis('off')

            #     axes[1, col].imshow(ogm_obs[agent_id][0, col], origin='lower')
            #     axes[1, col].set_title(f"ogm_pred[{agent_id}][0, {col}]")
            #     axes[1, col].axis('off')
            ...
            for agent_id in range(self.num_agents):
                ...
                for surround_agent_id in range(self.num_agents):
                    if agent_id != surround_agent_id:
                        ...
                        ego_ogm = fuse_ogm(ego_ogm, rob_ogm, translation_2d)
        ...
        for agent_id in range(self.num_agents):
            ...
            for t in range(self.vis_seq):
                ...
                ogm_batch = ogm_obs[:, agent_id, t]  # shape: (batch_size, t, H, W)
    ...
    def collect(self, step):
        ...
        value, cost_values, action, action_log_prob, rnn_states, rnn_states_critic, rnn_states_cost_critic \
            = self.trainer.policy.get_actions(np.concatenate(self.buffer.share_obs[step]),
                            np.concatenate(self.buffer.obs[step]),
                            np.concatenate(self.buffer.rnn_states[step]),
                            np.concatenate(self.buffer.rnn_states_critic[step]),
                            np.concatenate(self.buffer.rnn_states_cost_critic[step]),
        ...
        rnn_states_cost_critic = np.array(np.split(_t2n(rnn_states_cost_critic), self.n_rollout_threads))
        # rearrange action
        ...
        return values, cost_values, actions, action_log_probs, rnn_states, rnn_states_critic, actions_env, rnn_states_cost_critic

    def insert(self, data):
        obs, rewards, costs, dones, infos, values, cost_values, actions, action_log_probs, rnn_states, rnn_states_critic, rnn_states_cost_critic = data
        
        rnn_states[dones == True] = np.zeros(((dones == True).sum(), self.recurrent_N, self.hidden_size), dtype=np.float32)
        rnn_states_critic[dones == True] = np.zeros(((dones == True).sum(), *self.buffer.rnn_states_critic.shape[3:]), dtype=np.float32)
        rnn_states_cost_critic[dones == True] = np.zeros(((dones == True).sum(), *self.buffer.rnn_states_cost_critic.shape[3:]), dtype=np.float32)
        ...
        self.buffer.insert(share_obs, obs, rnn_states, rnn_states_critic, rnn_states_cost_critic, actions, action_log_probs, values, cost_values, rewards, costs, masks)
    ...
    def eval(self, total_num_steps):
        ...
        eval_episode_costs = np.zeros((self.n_eval_rollout_threads, self.num_agents))
        ...
        if self.use_gt_ogm:
            ...
            ogm_pred, costs = self.cooperative_prediction(lidar, vec_obs, is_eval=True)
        ...
        for eval_episode in range(self.all_args.eval_episodes):
            ...
            for eval_step in range(self.episode_length):
                ...
                frame_img = create_frame_in_memory(vec_obs[0,0,-1,:], human_obs[0,0,-1,:], robot_num=self.num_agents, human_num=self.human_num)
                render_frames.append(frame_img)
                ...
                for rollout in range(self.n_eval_rollout_threads):
                    for agent_id in range(self.num_agents): # n_eval_rollout
                        ...
                        eval_episode_costs[rollout, agent_id] += costs[rollout, agent_id]
                        # current_pos = eval_obs[rollout, agent_id, -27:-24]
                        # current_pos = eval_obs[rollout, agent_id, -9:-6]
                        current_pos = eval_obs[rollout, agent_id, -self.num_agents*9:-self.num_agents*9+3]
                        ...
                        if eval_dones[rollout, agent_id]:
                            ...
                            if eval_rewards[rollout, agent_id] == 10.0:
            ...
            gif_name = f"render_ep_{eval_episode}.gif"
            gif_path = os.path.join("./visualization", self.experiment_name, "prediction" if self.use_prediction else "perception_only")
            if not os.path.exists(gif_path):
                os.makedirs(gif_path, exist_ok=True)
            imageio.mimsave(os.path.join(gif_path, gif_name), render_frames, fps=20)
            print(f"[Render] Episode {eval_episode} saved GIF to: {gif_path}")
            render_frames.clear()

        eval_episode_rewards = np.array(eval_episode_rewards)
        eval_episode_rewards = np.mean(eval_episode_rewards, axis=0)
        eval_episode_costs = np.array(eval_episode_costs)
        eval_episode_costs = np.mean(eval_episode_costs, axis=0)
        log_message = f"total_num_steps: {total_num_steps}\n"
        log_message += f"eval average episode rewards: {eval_episode_rewards}\n"
        log_message += f"eval average episode costs: {eval_episode_costs}\n"
        ...
        for agent_id in range(self.num_agents):
            log_message += (
                ...
                f"Cost: {np.round(eval_episode_costs[agent_id], 2)}\n"
            )
        for agent_id in range(self.num_agents):
            log_message += (
