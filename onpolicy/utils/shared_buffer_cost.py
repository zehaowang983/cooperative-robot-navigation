...
class SharedReplayBufferCost(object):
    """
    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space):
        ...
        self.rnn_states_cost_critic = np.zeros_like(self.rnn_states)

        self.value_preds = np.zeros(
            (self.episode_length + 1, self.n_rollout_threads, num_agents, 1), dtype=np.float32)
        self.cost_value_preds = np.zeros_like(self.value_preds)
        self.returns = np.zeros_like(self.value_preds)
        self.cost_returns = np.zeros_like(self.value_preds)
    ...
    def insert(self, share_obs, obs, rnn_states_actor, rnn_states_critic, rnn_states_cost_critic, actions, action_log_probs,
               value_preds, cost_value_preds, rewards, costs, masks, bad_masks=None, active_masks=None, available_actions=None):
        """
        ...
        self.mean_costs[self.step] = np.mean(costs, axis=0, keepdims=True)
    ...
    def compute_returns(self, next_value, next_cost_value, value_normalizer=None):
        """
                ...
                for step in reversed(range(self.costs.shape[0])):
                    if self._use_popart or self._use_valuenorm:
                        if self.algo == "mat" or self.algo == "mat_dec":
                            ...
                            cost_delta = self.costs[step] + self.gamma * value_normalizer.denormalize(
                                self.cost_value_preds[step + 1]) * self.masks[step + 1] \
                                    - value_normalizer.denormalize(self.cost_value_preds[step])
                            cost_gae = cost_delta + self.gamma * self.gae_lambda * self.masks[step + 1] * cost_gae
                            self.cost_returns[step] = cost_gae + value_normalizer.denormalize(self.cost_value_preds[step])
                    else:
                        if self.algo == "mat" or self.algo == "mat_dec":
                            # rewards_t = self.rewards[step]
                            # mean_v_t = np.mean(self.value_preds[step], axis=-2, keepdims=True)
    ...
    def recurrent_generator(self, advantages, cost_advantages, num_mini_batch, data_chunk_length):
        """
        ...
        cost_advantages = _cast(cost_advantages)
        value_preds = _cast(self.value_preds[:-1])
        cost_value_preds = _cast(self.cost_value_preds[:-1])
        returns = _cast(self.returns[:-1])
        cost_returns = _cast(self.cost_returns[:-1])
        mean_costs = _cast(self.mean_costs)
        ...
        rnn_states_critic = self.rnn_states_critic[:-1].transpose(1, 2, 0, 3, 4).reshape(-1,
                                                                                         *self.rnn_states_critic.shape[
                                                                                          3:])
        rnn_states_cost_critic = self.rnn_states_cost_critic[:-1].transpose(1, 2, 0, 3, 4).reshape(-1,
                                                                                                 *self.rnn_states_cost_critic.shape[
                                                                                                  3:])
        ...
        for indices in sampler:
            ...
            for index in indices:
                ...
                mean_cost_batch.append(mean_costs[ind:ind + data_chunk_length])
            ...
            mean_cost_batch = np.stack(mean_cost_batch, axis=1)
            ...
            rnn_states_batch = np.stack(rnn_states_batch).reshape(N, *self.rnn_states.shape[3:])
            rnn_states_critic_batch = np.stack(rnn_states_critic_batch).reshape(N, *self.rnn_states_critic.shape[3:])
            rnn_states_cost_critic_batch = np.stack(rnn_states_cost_critic_batch).reshape(N, *self.rnn_states_cost_critic.shape[3:])
            ...
            cost_value_preds_batch = _flatten(L, N, cost_value_preds_batch)
            return_batch = _flatten(L, N, return_batch)
            cost_return_batch = _flatten(L, N, cost_return_batch)
            mean_cost_batch = _flatten(L, N, mean_cost_batch)
            ...
            yield share_obs_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, rnn_states_cost_critic_batch, actions_batch,\
                  ...
                  adv_targ, cost_adv_targ, available_actions_batch
