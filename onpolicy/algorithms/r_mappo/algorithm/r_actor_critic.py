    ...
    def forward(self, obs, rnn_states, masks, available_actions=None, deterministic=False):
        ...
        if self.use_traj:
            for agent_id in range(self.num_agents):
                ...
                ego_vec_obs = obs_i[:, :self.vis_seq*self.ego_vec_size].reshape(B, self.vis_seq, -1)[:, -1, :self.ego_vec_size]
                # other_vec_obs = obs_i[:, self.ego_vec_size:self.ego_vec_size+self.other_vec_size]
                human_vec_obs = obs_i[:, self.vis_seq*(self.ego_vec_size+self.other_vec_size):].reshape(B, self.vis_seq, self.human_vec_size)[:, -1, :]
                relative_human_vec_obs = human_vec_obs - ego_vec_obs.repeat(1,12)
                ego_vec_x = self.ego_vec_base(ego_vec_obs)
                # other_vec_x = self.other_vec_base(other_vec_obs)
                human_vec_x = self.human_vec_base(relative_human_vec_obs)

                actor_features = torch.cat([ego_vec_x, human_vec_x], dim=-1)
            ...
            for agent_id in range(self.num_agents):
                ...
                if self.use_prediction:
                    vis_obs = obs_i[:, :(self.vis_seq+1)*c*h*w].reshape(B, self.vis_seq+1, c, h, w)
                    vec_obs = obs_i[:, (self.vis_seq+1)*c*h*w:].reshape(B, self.vis_seq, (self.ego_vec_size+self.other_vec_size))
                else:
                ...
                # for i in range(B):
                #     plt.imshow(conv_lstm_output[0][i, 0, 0].cpu().detach().numpy(), origin='lower')
                #     plt.show()

                #     plt.imshow(conv_lstm_output[0][i, -1, 0].cpu().detach().numpy(), origin='lower')
                #     plt.show()

                actor_features = torch.cat([vis_x, ego_vec_x, other_vec_x], dim=-1)
                # actor_features = torch.cat([vis_x, ego_vec_x], dim=-1)
    ...
    def evaluate_actions(self, obs, rnn_states, action, masks, available_actions=None, active_masks=None):
        ...
        if self.use_traj:
            ...
            for agent_id in range(self.num_agents):
                ...
                # for i in range(B):
                #     plt.imshow(conv_lstm_output[0].mean(dim=1)[i, 0].cpu().detach().numpy(), origin='lower')
                #     plt.show()
...
class R_Critic(nn.Module):
    ...
    def forward(self, cent_obs, rnn_states, masks):
        ...
        if self.use_traj:
            ...
            ego_vec_obs = cent_obs[:, :self.vis_seq*self.ego_vec_size].reshape(B, self.vis_seq, self.ego_vec_size)[:, -1, :]
            human_vec_obs = cent_obs[:, self.vis_seq*(self.ego_vec_size+self.other_vec_size):].reshape(B, self.vis_seq, self.human_vec_size)[:, -1, :]
            relative_human_vec_obs = human_vec_obs - ego_vec_obs.repeat(1,12)
            
            ego_vec_x = self.ego_vec_base(ego_vec_obs)
            human_vec_x = self.human_vec_base(relative_human_vec_obs)
            critic_features = torch.cat([ego_vec_x, human_vec_x], dim=-1)
        else:
            c, w, h = self.vis_size
            ...
            if self.use_prediction:
                vis_obs = cent_obs[:, :(self.vis_seq+1)*c*h*w].reshape(B, self.vis_seq+1, c, h, w)
                vec_obs = cent_obs[:, (self.vis_seq+1)*c*h*w:].reshape(B, self.vis_seq, (self.ego_vec_size+self.other_vec_size))
            else:
            ...
            # for i in range(B):
            #     plt.imshow(conv_lstm_output[0].mean(dim=1)[i*3, 0].cpu().detach().numpy(), origin='lower')
            #     plt.title(f"batch {i}")
            #     plt.show()
            ...
            # critic_features = torch.cat([vis_x, ego_vec_x], dim=-1)

            # c, w, h = self.vis_size
            # B, cent_obs_dim = cent_obs.shape
            # single_agent_obs_dim = cent_obs_dim // self.num_agents
            # vis_obs_dim = self.vis_seq * c * h * w
            # vec_obs_dim = self.ego_vec_size + self.other_vec_size

            # # Process each agent's observation
            # agent_features = []
            # for i in range(3):  # Iterate through the three agents
            #     start_idx = i * single_agent_obs_dim
            #     end_idx = (i + 1) * single_agent_obs_dim

            #     agent_obs = cent_obs[:, start_idx:end_idx]

            #     # Extract visual and vector observations for the current agent
            #     vis_obs = agent_obs[:, :vis_obs_dim].reshape(B, self.vis_seq, c, h, w)
            #     vec_obs = agent_obs[:, vis_obs_dim:].reshape(B, self.vis_seq, vec_obs_dim)

            #     ego_vec_obs = vec_obs[:, -1, :self.ego_vec_size]
            #     other_vec_obs = vec_obs[:, -1, self.ego_vec_size:]
            #     relative_other_vec_obs = other_vec_obs - ego_vec_obs.repeat(1, 2)

            #     # Process visual observations
            #     conv_lstm_output, _ = self.conv_lstm(vis_obs)
            #     vis_x = conv_lstm_output[0].mean(dim=1).view(B, -1) #Corrected this line

            #     # Process vector observations
            #     ego_vec_x = self.ego_vec_base(ego_vec_obs)
            #     other_vec_x = self.other_vec_base(relative_other_vec_obs)

            #     # Concatenate features for the current agent
            #     agent_feature = torch.cat([vis_x, ego_vec_x, other_vec_x], dim=-1)
            #     agent_features.append(agent_feature)

            # # Concatenate the features of all agents
            # critic_features = torch.cat(agent_features, dim=-1)
