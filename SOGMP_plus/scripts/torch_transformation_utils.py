...
def warp_affine(
    ...
    Transform the src based on transformation matrix M.
    Args:
        src : torch.Tensor - Input feature map with shape (B,C,H,W).
        M : torch.Tensor - Transformation matrix with shape (B,2,3).
        dsize : tuple - Tuple of output image H_out and W_out.
        mode : str - Interpolation methods for F.grid_sample.
        padding_mode : str - Padding methods for F.grid_sample.
        align_corners : boolean - Parameter of F.affine_grid.

    Returns:
        Transformed features with shape (B,C,H,W).
    """

    device = src.device  # Ensure everything is on the same device as src
    B, C, H, W = src.size()
    
    
    # Convert 2x3 affine matrix to 3x3 homography
    M_3x3 = convert_affinematrix_to_homography(M.to(device))
    ...
    # Adjust tensor type for grid sampling based on grid type
